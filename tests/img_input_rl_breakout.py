...
env = gym.make('BreakoutDeterministic-v4')  # https://gym.openai.com/envs/Breakout-v0/
env.reset()

# To obtain a "screenshot" to use as input for the model
screenshot = env.render(mode='rgb_array')  # 210x160
...
def build_model(width, height, num_actions):  # todo: conv model (with rnn)
    ...
    # model = Model(inputs=[frames_input, actions_input], outputs=merged_out)
